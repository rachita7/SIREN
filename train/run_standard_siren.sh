...
# C = inverse L1 strength. Default grid suits 4096-dim residual_mean; the
# 14336-dim mlpneuron_mean space needs weaker regularization, e.g.
# C_VALUES="1000.0 5000.0 20000.0" (small C collapses those probes).
...
# Output names get this suffix so runs never overwrite each other:
#   probes/{MODEL}_general_probes{SUFFIX}.pkl
#   probes/optuna/{MODEL}_general{SUFFIX}/
# Defaults: "" for residual_mean (existing paths keep working),
# "-std-<rep_type>" otherwise. Override with OUTPUT_SUFFIX for variants.
if [ -z "${OUTPUT_SUFFIX+x}" ]; then
    if [ "$REP_TYPES" = "residual_mean" ]; then
        OUTPUT_SUFFIX=""
    else
        OUTPUT_SUFFIX="-std-${REP_TYPES// /+}"
    fi
fi
...
echo "Alpaca train file: ${SIREN_ALPACA_TRAIN:-alpaca_train}.csv"
